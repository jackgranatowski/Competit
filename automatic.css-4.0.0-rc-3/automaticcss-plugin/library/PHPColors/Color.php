...
class Color
{
    ...
    public function __construct(string $hex)
    {
        ...
        $this->_oklch = self::hexToOklch($color);
    }

    public static function hexToOklch(string $hex): array
    {
        $color = ltrim(self::sanitizeHex($hex), '#');
        if (strlen($color) === 3) {
            $color = $color[0].$color[0].$color[1].$color[1].$color[2].$color[2];
        }

        // 1. sRGB → linear RGB
        $toLinear = function (float $v): float {
            return ($v <= 0.04045) ? $v / 12.92 : pow(($v + 0.055) / 1.055, 2.4);
        };
        $R = $toLinear(hexdec(substr($color, 0, 2)) / 255);
        $G = $toLinear(hexdec(substr($color, 2, 2)) / 255);
        $B = $toLinear(hexdec(substr($color, 4, 2)) / 255);
    ...
    public static function hslToOklch(array $hsl): array
    {
        $hex = self::hslToHex($hsl);
    ...
    public function getRgb(): array
    {
     ...
     /**
     * Returns the OKLCH array of your color
     */
    public function getOklch(): array
    {
        return $this->_oklch;
    }
