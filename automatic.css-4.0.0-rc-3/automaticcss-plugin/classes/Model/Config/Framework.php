	...
	public function get_color_palettes( $options = array() ) {
		...
		if ( array_key_exists( 'category_groups', $color_category ) ) {
			foreach ( $color_category['category_groups'] as $main_color_name => $category_group ) {
				...
				// Handle flat vars directly on the group (refactored palette format).
				if ( array_key_exists( 'vars', $category_group ) ) {
					foreach ( $category_group['vars'] as $variable => $variable_options ) {
						if ( $variable !== $main_color_name ) {
