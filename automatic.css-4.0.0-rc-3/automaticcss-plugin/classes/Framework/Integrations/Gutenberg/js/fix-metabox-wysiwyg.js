(function () {
	window.addEventListener("load", () => {
		if (!automatic_css_metabox_options) {
			console.error(
				"automatic_css_metabox_options is not defined. Please check if the plugin is activated."
			);
			return;
		}
		let root_font_size = automatic_css_metabox_options.root_font_size + "%";
		...
		metabox_iframes.forEach((iframe) => {
			...
			const iframe_headings = iframe.contentDocument.querySelectorAll(
				"h1, h2, h3, h4, h5, h6"
			);
